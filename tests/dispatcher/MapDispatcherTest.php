...
use \PEIP\Dispatcher\MapDispatcher as PEIP_Map_Dispatcher;
...
class MapDispatcherTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testNotifyUntil()
    {
        ...
        $subject = 'foo';
        $test = $this;
        $test->assertFalse((bool) $this->dispatcher->notifyUntil($eventName, $subject));
        for ($x = 1; $x < $handlerNr; $x++) {
            $handlersCalled[$x] = false;
            $this->dispatcher->connect($eventName, function ($sub) use ($test, $x,$subject) {
                $test->assertEquals($subject, $sub);
    ...
    public function testGetListeners()
    {
        ...
        $handlers = [];
        $test = $this;
        $test->assertEquals([], $this->dispatcher->getListeners($eventName));
        for ($x = 1; $x < $handlerNr; $x++) {
            $handlersCalled[$x] = false;
            $handlers[$x] = function () {
            };
